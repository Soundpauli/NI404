  ...
  Mode set_Wav = { "SET_WAV", { 0, 1, 1, 0 }, { 0, maxFolders, 999, 999 }, { 0, 0, 1, 999 } };
  ...
  void showWave() {
    ...
    if (isEncoder4Defined) {
      if ((currentMode->pos[3]) != SMP.seekEnd && sampleIsLoaded) {
        ...
        if (SD.exists(OUTPUTf)) {
          ...
          showNumber(SMP.wav, col_Folder[SMP.folder - 1], 0);
          if (!sampleLengthSet) previewSample(getFolderNumber(SMP.wav), getFileNumber(SMP.wav), false);
        } else {
          showIcons("helper_select", col[SMP.y - 1]);
    ...
    if ((currentMode->pos[0]) - 1 != SMP.seek && sampleIsLoaded) {
      ...
      if (SD.exists(OUTPUTf)) {
        ...
        showIcons("helper_folder", CRGB(10, 30, 0));
        showNumber( SMP.wav, col_Folder[SMP.folder - 1], 0);
        previewSample(getFolderNumber(SMP.wav), getFileNumber(SMP.wav), false);
      } else {
        showIcons("helper_select", col[SMP.y - 1]);
        showIcons("helper_load", CRGB(0, 0, 0));
        showIcons("helper_folder", CRGB(10, 10, 0));
    ...
    if (currentMode->pos[2] != SMP.wav) {
     ...
     // if (SD.exists(OUTPUTf)) {
        ...
        showIcons("helper_folder", CRGB(10, 30, 0));
        showNumber(SMP.wav, col_Folder[SMP.folder - 1], 0);
        previewSample(getFolderNumber(SMP.wav), getFileNumber(SMP.wav), true);
        sampleIsLoaded = true;
