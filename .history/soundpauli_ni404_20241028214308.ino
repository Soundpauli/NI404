...
// return the file number in the folder, so 113 = 13, 423 = 23
  int folder = getFoldeNumber(value);
  ...
  void showWave() {
    ...
    if (currentMode->pos[2] != SMP.wav) {
      ...
      Serial.println("File: " + String(SMP.wav));
      ...
      if (((SMP.folder - 1) * 100) + SMP.wav < 100) showIcons("icon_sample", col[SMP.y - 1]);
