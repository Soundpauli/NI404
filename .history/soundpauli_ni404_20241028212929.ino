  ...
  EXTMEM int lastFile[maxFolders] = { 0, 0, 0, 0, 0, 0, 0, 0, 0 };
  ...
  void setup() {
    ...
    for (int f = 0; f <= maxFolders; f++) {
      for (unsigned int i = 1; i < 99; i++) {
        ...
        if (SD.exists(OUTPUTf)) { lastFile[f] = i;}
      }
    }
  ...
  void showWave() {
    ...
    if (currentMode->pos[2] != SMP.wav) {
      ...
      Serial.print("FILE:");
      Serial.print(getFolderNumber( SMP.wav));
      Serial.print(" / ");
      Serial.print(getFileNumber( SMP.wav));
...
void setLastFile(unsigned int folder) { 
  encoders[1].write((folder * 4)-1);
  Serial.println("LastFile: " + String(lastFile[folder]));
  SMP.wav[SMP.currentChannel] = lastFile[folder];
  encoders[2].write((lastFile[folder] * 4));
  //currentMode->pos[2] = lastFile[folder];
}
