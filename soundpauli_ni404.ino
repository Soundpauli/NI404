...
const int numPulsesForAverage = 24; // Number of pulses to average over


volatile bool waitForFourBars = false;
volatile int pulseCount = 0;
const int pulsesPerBar = 24 * 4; // 24 pulses per quarter note, 4 quarter notes per bar
const int barsToWait = 2;
const int totalPulsesToWait = pulsesPerBar * barsToWait;
...
struct Device {
  int singleMode; // single Sample Mode
  int currentChannel;
  int vol; //volume
  int bpm; //bpm
  int velocity; //velocity
  int page; //current page
  int edit; // edit mode or plaing mode?
  int file; //current selected save/load id
  int pack; // current selected samplepack id
  int wav; // current selected sample id
  int folder; //current selected folder id
...
void FastLEDclear() {
  FastLED.clear();
// do not clear if not shown yet
  /*if (!cleared){
    FastLED.clear();
    // save that it was cleared
    cleared = true;
    }
    */
}
...
void FastLEDshow() {
  FastLED.show();
  // only show if FastLEDclear was called before
  /*notclear++;
  
  if (cleared || notclear>2) {
    FastLED.show();
    notclear=0;
    cleared = false;
  }*/
...
void toggleMute() {
  if (mute[SMP.y - 1]) {
    mute[SMP.y - 1] = false;
    envelopes[SMP.y - 1]->hold(2.5);
    envelopes[SMP.y - 1]->decay(35);
    envelopes[SMP.y - 1]->release(300);

  } else {
    //wenn leer oder nicht gemuted:
    mute[SMP.y - 1] = true;

    envelopes[SMP.y - 1]->hold(0);
    envelopes[SMP.y - 1]->decay(0);
    envelopes[SMP.y - 1]->release(0);
...
void drawPages() {
  ...
  for (int p = 1; p <= maxPages; p++) {  // Assuming maxPages is 8
    ...
    if (SMP.page == p && SMP.edit == p) {
      if (isPlaying){
        ledColor = CRGB(0, 255, 0);  // Bright green if active page is same as edit page and playing
      }else{
        ledColor = CRGB(255, 0, 0);  // Bright red if active page is same as edit page and stopped
      }
    } else if (SMP.page == p) {
      if (isPlaying){
        ledColor = CRGB(0, 15, 0);  // White green for current page
      }else{
        ledColor = CRGB(15, 0, 0);  // White green for current page
      }
    } else {
      // Determine color for non-current pages
      if (SMP.edit == p) {
        ledColor = SMP.page == p ? CRGB(5, 0, 5) : CRGB(20, 0, 20);  // Bright violett if active pattern is the same as edit, Dark vilett otherwise
      } else {
